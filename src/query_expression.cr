...
module Interro
  struct QueryExpression
    ...
    # Renders this fragment on its own, numbering its placeholders from `$1` and discarding the values.
    # Only meaningful for inspecting a fragment: one that is about to be executed must be rendered into the query's args array so its numbers match what gets bound.
